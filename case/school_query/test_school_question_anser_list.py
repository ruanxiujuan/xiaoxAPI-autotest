...
class TestSchoolQuestionAndAnserList(unittest.TestCase):
    ...
    def test_school_question_anser_list(self):
        '''
        测试热门院校接口
        '''
        url = "/question/questionAndAnserlist"
        headers = {
            "vcode":self.vcode,
        ...
        # 断言：获取院校问校代信息并判断是否存在，存在则打印
        try:
            value = res.json()['body']['value']
            if list:
                logging.info("院校详情问校代信息~")
                logging.info(json.dumps(value, ensure_ascii=False, sort_keys=True, indent=2))
            else:
                logging.info("无院校问校代信息~")
