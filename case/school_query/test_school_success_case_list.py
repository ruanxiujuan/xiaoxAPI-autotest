...
class TestSchoolSuccessCaseList(unittest.TestCase):
    ...
    def test_school_success_case_list(self):
        '''
        测试热门院校接口
        '''
        url = "/successCase/successCaseList"
        headers = {
            "vcode":self.vcode,
        ...
        try:
            successCaseList = res.json()['body']['successCaseList']
            if list:
                logging.info("院校详情全部成功案例信息~")
                logging.info(json.dumps(successCaseList, ensure_ascii=False, sort_keys=True, indent=2))
            else:
                logging.info("无院校成功案例信息~")
