...
class TestSchoolRankAustralia(unittest.TestCase):
    ...
    def test_school_rank_01(self):
        ...
        params = {
            ...
            "rankType": 3,           # 排名类型，0-QS(默认),1-US NEWS, 2-TIMES, 3-Maclean
    ...
    def test_school_rank_07(self):
        ...
        # 断言：获取院校排名列表信息
        schoolRankList = res.json()['body']['schoolRankList']
        localTypeName = jsonpath.jsonpath(res.json(), "$.body.schoolRankList[*].localTypeName")
        try:
            if schoolRankList:
                self.assertIsNotNone(schoolRankList)
                logging.info("院校排名信息为：")
                for i in schoolRankList:
                    logging.info(i)
                nationalRank = []
                for j in localTypeName:
                    nationalRank.append(j)
                logging.info("本国排名分类", set(nationalRank))
