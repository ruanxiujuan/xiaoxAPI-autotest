...
class TestSchoolNoticeList(unittest.TestCase):
    ...
    def test_school_notice_list(self):
        '''
        测试热门院校接口
        '''
        url = "/school/notice"
        ...
        # 断言：获取院校通知列表信息并判断是否存在，存在则打印
        try:
            noticeList = res.json()['body']['noticeList']
            if list:
                logging.info("院校详情全部通知信息~")
                logging.info(json.dumps(noticeList, ensure_ascii=False, sort_keys=True, indent=2))
            else:
                logging.info("无院校通知信息~")
